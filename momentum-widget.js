(async function () {
  ...
  async function fetchVWAPandTime(ticker) {
    try {
      ...
      let tpv = 0, vol = 0;
      candles
        .filter(bar => bar.date.startsWith(today))
        .forEach(bar => {
      ...
      const computedVWAP = vol ? tpv / vol : null;
      console.log(`${ticker} VWAP:`, computedVWAP);
      ...
      console.warn(`VWAP+Time fetch failed for ${ticker}`, err);
      return { vwap: null, time: '—' };
    }
  }
  ...
  async function buildTable(data, title, strategyLabel) {
    ...
    for (const r of data) {
      const tr      = tbody.insertRow(),
            ...
            volume  = r.volume   ?? 0,
            flt     = r.float    ?? 0,
            floatPct= flt>0? (volume/flt)*100 : null,
            atr     = r.atr      ?? null,
            gapPct  = r.gap_pct  ?? 0;
      ...
      const price  = r.price ?? close ?? 0,
            change = (open || open===0)
                     ? ((price - open)/open)*100
                     : null,
            deviation = (vwap || vwap===0)
                        ? ((price - vwap)/vwap)*100
                        : null,
            rvol    = r.rvol ?? r.rvol_value ?? 0;
      ...
      console.log(ticker, {change, rvol, deviation, signal});

      const cells = [
        strategy, time, arrow(change), ticker, fmt(price),
        fmt(change), fmt(gapPct), abbrMil(volume),
        fmt(floatPct,1), fmt(atr),
